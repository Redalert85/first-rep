...
class LegalKnowledgeGraph:
    ...
    def _initialize_all_subjects(self):
        ...
        # Advanced concepts for key MBE subjects (30 additional)
        self._add_criminal_procedure_advanced()
        self._add_real_property_advanced()
...
# Adds 58-98 concepts for complete NCBE coverage
    ...
    def _add_criminal_procedure_advanced(self):
        """Add 15 additional advanced criminal_procedure concepts"""
        concepts = [
            KnowledgeNode(
                concept_id="criminal_procedure_consent_searches",
                name="Consent Searches",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="Voluntary consent eliminates warrant requirement; can be limited or withdrawn; third party with common authority can consent",
                elements=['Voluntariness', 'Scope of consent', 'Third party authority', 'Apparent authority'],
                policy_rationales=['Efficient law enforcement', 'Waiver of rights'],
                common_traps=['Missing that consent must be voluntary, not coerced', 'Forgetting third party can consent to shared spaces', 'Not limiting consent to areas actually consented to'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_protective_sweeps",
                name="Protective Sweeps",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="Quick cursory visual inspection of areas where person may be hiding; requires reasonable suspicion during lawful arrest in home",
                elements=['Lawful arrest', 'Reasonable suspicion', 'Cursory inspection', 'Areas of concealment'],
                policy_rationales=['Officer safety'],
                common_traps=['Extending sweep beyond areas where person could hide', 'Conducting sweep without specific articulable facts', 'Searching containers during protective sweep'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_cell_phone_searches",
                name="Cell Phone Searches (Riley)",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="Warrant required to search cell phone even incident to arrest; digital data gets full Fourth Amendment protection (Riley v. California)",
                elements=['Warrant required', 'No search incident exception', 'Exigent circumstances still apply', 'Third party doctrine limits'],
                policy_rationales=['Privacy in digital age', 'Vast amount of personal data'],
                common_traps=['Applying search incident to arrest to cell phones (Riley says no)', 'Missing that exigency like remote wiping may justify warrantless search', 'Forgetting third party doctrine for cloud data'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_dog_sniffs",
                name="Dog Sniffs",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="Dog sniff is not a search at airports or traffic stops; but sniff at home requires warrant (Florida v. Jardines)",
                elements=['Public place permitted', 'Traffic stop OK', 'Home curtilage requires warrant', 'Reliable dog required'],
                policy_rationales=['Binary nature reveals only contraband'],
                common_traps=['Allowing dog sniff at home without warrant (Jardines)', 'Extending traffic stop unreasonably for dog (Rodriguez)', 'Using unreliable dog sniff as probable cause'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_good_faith_exception",
                name="Good Faith Exception",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="Evidence not excluded if officer reasonably relied on warrant later found defective; also applies to binding appellate precedent and statute",
                elements=['Reasonable reliance', 'Objectively reasonable', 'Magistrate independence', 'Four exceptions to exception'],
                policy_rationales=['Deterrence rationale not served by excluding'],
                common_traps=['Applying when affidavit so lacking probable cause no reasonable officer would rely', 'Missing that misleading or false affidavit negates good faith', 'Forgetting magistrate must be neutral and detached'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_knock_and_announce",
                name="Knock and Announce Rule",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="Officers must knock and announce before executing warrant unless exigent circumstances; violation does not require exclusion (Hudson)",
                elements=['Knock and announce', 'Wait reasonable time', 'Exigent exceptions', 'No exclusion remedy'],
                policy_rationales=['Privacy protection', 'Prevent violence'],
                common_traps=['Thinking violation requires exclusion (Hudson says no)', 'Missing that no-knock warrant can be authorized', 'Forgetting reasonable wait time depends on circumstances'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_pretrial_detention",
                name="Pretrial Detention & Bail",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="No constitutional right to bail; pretrial detention constitutional if flight risk or danger to community; no excessive bail for bailable offenses",
                elements=['No right to bail', 'Flight risk', 'Danger to community', 'Excessive bail clause'],
                policy_rationales=['Ensure appearance', 'Public safety'],
                common_traps=['Thinking Constitution guarantees bail', 'Missing that capital offenses not bailable', 'Forgetting preventive detention OK if findings made'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_speedy_trial",
                name="Speedy Trial Right",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="Sixth Amendment right to speedy trial; analyze length of delay, reason for delay, defendant's assertion, and prejudice (Barker factors)",
                elements=['Length of delay', 'Reason for delay', 'Assertion of right', 'Prejudice'],
                policy_rationales=['Prevent oppressive incarceration', 'Minimize anxiety', 'Limit impairment of defense'],
                common_traps=['Missing that right attaches at arrest or charge', 'Forgetting that remedy is dismissal with prejudice', 'Not balancing all four Barker factors'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_double_jeopardy",
                name="Double Jeopardy",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="No second prosecution after acquittal or conviction for same offense; separate sovereigns exception; jeopardy attaches when jury sworn",
                elements=['Same offense test (Blockburger)', 'Acquittal bar', 'Conviction bar', 'Separate sovereigns'],
                policy_rationales=['Finality', 'Prevent government harassment'],
                common_traps=['Missing separate sovereigns (federal and state can both prosecute)', 'Forgetting jeopardy attaches when jury sworn or first witness in bench trial', 'Confusing collateral estoppel with double jeopardy'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_right_to_jury",
                name="Right to Jury Trial",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="Right to jury for crimes with potential sentence over 6 months; 6 person jury minimum; unanimity required in federal court",
                elements=['Over 6 months potential', 'Minimum 6 jurors', 'Unanimity requirement', 'Waiver'],
                policy_rationales=['Community judgment', 'Check on government'],
                common_traps=['Applying to petty offenses under 6 months', 'Missing that states can use non-unanimous verdicts (overruled by Ramos)', 'Forgetting defendant can waive jury'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_confrontation",
                name="Confrontation Clause",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="Right to confront witnesses; testimonial hearsay barred unless witness unavailable and prior cross-examination (Crawford)",
                elements=['Testimonial statements', 'Unavailability', 'Prior cross-examination', 'Face-to-face right'],
                policy_rationales=['Test reliability through cross-examination'],
                common_traps=['Missing that non-testimonial hearsay governed by hearsay rules, not Crawford', 'Forgetting dying declarations may be admitted despite Confrontation Clause', 'Confusing unavailability requirements'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_effective_counsel",
                name="Ineffective Assistance of Counsel",
                subject="criminal_procedure",
                difficulty=5,
                rule_statement="Counsel ineffective if performance deficient and prejudice resulted (Strickland); prejudice presumed for actual conflict, complete denial, or state interference",
                elements=['Deficient performance', 'Prejudice', 'Objective standard', 'Presumed prejudice categories'],
                policy_rationales=['Fair trial guarantee'],
                common_traps=['Missing that strategic decisions get deference', 'Forgetting prejudice means reasonable probability of different result', 'Not recognizing presumed prejudice situations'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_guilty_pleas",
                name="Guilty Pleas & Waivers",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="Plea must be knowing, voluntary, intelligent; court must advise of rights waived and consequences; ineffective assistance can invalidate",
                elements=['Knowing and voluntary', 'Nature of charge', 'Consequences', 'Rights waived'],
                policy_rationales=['Efficient adjudication', 'Defendant autonomy'],
                common_traps=['Missing that court must personally address defendant', 'Forgetting Padilla requires advice on deportation', 'Not knowing breach of plea deal allows withdrawal'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_sentencing",
                name="Sentencing (Apprendi/Blakely)",
                subject="criminal_procedure",
                difficulty=4,
                rule_statement="Any fact increasing penalty beyond statutory maximum must be found by jury beyond reasonable doubt (except prior convictions)",
                elements=['Statutory maximum', 'Jury finding required', 'Beyond reasonable doubt', 'Prior conviction exception'],
                policy_rationales=['Jury right protection'],
                common_traps=['Missing that prior convictions are excepted (Almendarez-Torres)', 'Confusing with sentencing guidelines (Booker made advisory)', 'Forgetting applies to mandatory minimums too (Alleyne)'],
            ),
            KnowledgeNode(
                concept_id="criminal_procedure_appeals",
                name="Appeals & Post-Conviction",
                subject="criminal_procedure",
                difficulty=3,
                rule_statement="No constitutional right to appeal; if state provides, must provide counsel; harmless error doesn't reverse; habeas limited to federal questions",
                elements=['No constitutional right', 'Right to counsel if appeal provided', 'Harmless error', 'Habeas corpus'],
                policy_rationales=['Finality vs. accuracy'],
                common_traps=['Thinking appeal is constitutionally required', 'Missing that structural errors never harmless', 'Forgetting AEDPA limits habeas review of state convictions'],
            ),
        ]
        for node in concepts:
            self.nodes[node.concept_id] = node

    def _add_real_property_advanced(self):
        """Add 15 additional advanced real_property concepts"""
        concepts = [
            KnowledgeNode(
                concept_id="real_property_rule_against_perpetuities",
                name="Rule Against Perpetuities (RAP)",
                subject="real_property",
                difficulty=5,
                rule_statement="No interest is good unless it must vest, if at all, not later than 21 years after some life in being at creation of interest",
                elements=['Lives in being', '21 years', 'Must vest or fail', 'Measuring lives', 'Saving clauses'],
                policy_rationales=['Prevent dead hand control', 'Encourage alienability'],
                common_traps=['Missing that RAP applies at creation, not when interest might vest', 'Forgetting to look for validating or invalidating lives', 'Not recognizing class gifts close when class closes'],
            ),
            KnowledgeNode(
                concept_id="real_property_waste",
                name="Waste Doctrine",
                subject="real_property",
                difficulty=4,
                rule_statement="Life tenant or other limited owner must not commit affirmative waste (damage), permissive waste (failure to maintain), or ameliorative waste (change character even if improves value)",
                elements=['Affirmative waste', 'Permissive waste', 'Ameliorative waste', 'Open mines doctrine'],
                policy_rationales=['Protect future interest holders'],
                common_traps=['Missing open mines doctrine exception', 'Forgetting ameliorative waste can be actionable even if value increases', 'Not knowing life tenant must pay taxes and mortgage interest'],
            ),
            KnowledgeNode(
                concept_id="real_property_shelter_rule",
                name="Shelter Rule (Recording Acts)",
                subject="real_property",
                difficulty=4,
                rule_statement="One who takes from BFP is sheltered by BFP's status even if not a BFP themselves; allows BFP to convey good title",
                elements=['BFP protection transfers', 'Notice irrelevant for transferee', 'Prevents circuity', 'Market title stability'],
                policy_rationales=['Allow BFP to alienate property', 'Market stability'],
                common_traps=['Thinking each purchaser must independently qualify as BFP', 'Missing that shelter does not apply if original grantor reacquires', 'Forgetting shelter applies in chain from BFP'],
            ),
            KnowledgeNode(
                concept_id="real_property_wild_deeds",
                name="Wild Deeds & Chain of Title",
                subject="real_property",
                difficulty=4,
                rule_statement="Deed recorded outside chain of title (wild deed) does not provide constructive notice; must be able to trace through recorded documents",
                elements=['Chain of title', 'Grantor-grantee index', 'Wild deed', 'Late recorded deeds'],
                policy_rationales=['Searchability of records'],
                common_traps=['Missing that late recorded deed may not provide notice', 'Forgetting deed from stranger provides no notice', 'Not checking both grantor and grantee indices'],
            ),
            KnowledgeNode(
                concept_id="real_property_marketable_title",
                name="Marketable Title",
                subject="real_property",
                difficulty=4,
                rule_statement="Seller must provide title free from reasonable doubt; encumbrances, zoning violations, adverse possession claims render title unmarketable",
                elements=['Free from reasonable doubt', 'Encumbrances disclosed', 'Zoning violations', 'Time for curing'],
                policy_rationales=['Buyer protection', 'Fair dealing'],
                common_traps=['Missing that zoning violation makes title unmarketable', 'Forgetting that recorded easement is encumbrance unless disclosed', 'Confusing marketable title with insurable title'],
            ),
            KnowledgeNode(
                concept_id="real_property_merger_doctrine",
                name="Merger Doctrine",
                subject="real_property",
                difficulty=3,
                rule_statement="Upon delivery of deed, contract terms merge into deed; any contract warranties not in deed are extinguished",
                elements=['Contract extinguished', 'Deed controls', 'Collateral promises survive', 'Fraud exception'],
                policy_rationales=['Finality of transaction'],
                common_traps=['Missing that collateral promises (like repair obligations) survive merger', 'Forgetting fraud claims survive', 'Thinking buyer can sue on contract after accepting deed'],
            ),
            KnowledgeNode(
                concept_id="real_property_equitable_conversion",
                name="Equitable Conversion",
                subject="real_property",
                difficulty=4,
                rule_statement="After contract signed, buyer has equitable title (real property), seller has legal title (personal property); risk of loss on buyer",
                elements=['Equitable title to buyer', 'Legal title to seller', 'Risk of loss', 'Insurance implications'],
                policy_rationales=['Equity treats as done what ought to be done'],
                common_traps=['Missing that risk of loss on buyer unless seller fault', 'Forgetting insurance implications', 'Not knowing Uniform Vendor and Purchaser Risk Act changes rule'],
            ),
            KnowledgeNode(
                concept_id="real_property_after_acquired_title",
                name="After-Acquired Title (Estoppel by Deed)",
                subject="real_property",
                difficulty=4,
                rule_statement="If grantor conveys property they don't own, but later acquires it, title automatically passes to grantee; applies to warranty deeds",
                elements=['Grantor without title', 'Later acquisition', 'Automatic passage', 'Warranty deed required'],
                policy_rationales=['Enforce grantor warranties', 'Protect grantee expectations'],
                common_traps=['Applying to quitclaim deeds (only warranty deeds)', 'Missing that BFP from grantor may defeat grantee', 'Forgetting that recording affects priority'],
            ),
            KnowledgeNode(
                concept_id="real_property_lateral_support",
                name="Lateral & Subjacent Support",
                subject="real_property",
                difficulty=3,
                rule_statement="Landowner entitled to lateral support from adjacent land in natural condition; subjacent support from mineral rights holder; strict liability for removal",
                elements=['Lateral support right', 'Subjacent support', 'Natural condition', 'Strict liability'],
                policy_rationales=['Prevent land collapse'],
                common_traps=['Missing that right only to land in natural state (not buildings)', 'Forgetting can recover for buildings if would have collapsed anyway', 'Confusing with negligence standard for excavation'],
            ),
            KnowledgeNode(
                concept_id="real_property_water_rights",
                name="Water Rights",
                subject="real_property",
                difficulty=4,
                rule_statement="Riparian rights: reasonable use of water adjacent to property; prior appropriation (western): first in time, first in right; groundwater varies by jurisdiction",
                elements=['Riparian rights', 'Prior appropriation', 'Reasonable use', 'Natural flow theory'],
                policy_rationales=['Efficient water allocation'],
                common_traps=['Confusing riparian (eastern) with prior appropriation (western)', 'Missing that riparian right lost if land sold to non-riparian', 'Forgetting groundwater rules differ from surface water'],
            ),
            KnowledgeNode(
                concept_id="real_property_nuisance",
                ...
                subject="real_property",
                difficulty=3,
                rule_statement="Substantial and unreasonable interference with use and enjoyment of land; balance gravity of harm against utility of conduct",
                elements=['Substantial interference', 'Unreasonable', 'Balancing test', 'Coming to nuisance'],
                policy_rationales=['Reasonable use of property'],
                common_traps=['Missing that coming to nuisance not a defense (just a factor)', 'Forgetting hypersensitive plaintiff not protected', 'Confusing with trespass (nuisance is intangible interference)'],
            ),
            KnowledgeNode(
                concept_id="real_property_common_interest_communities",
                name="Common Interest Communities (HOAs)",
                subject="real_property",
                difficulty=3,
                rule_statement="CC&Rs run with land if properly recorded; HOA can enforce reasonable restrictions; amendments require specified vote; must be uniformly enforced",
                elements=['CC&Rs', 'Enforcement powers', 'Amendment procedures', 'Reasonableness test'],
                policy_rationales=['Community standards', 'Property values'],
                common_traps=['Missing that unreasonable restrictions unenforceable', 'Forgetting selective enforcement defense', 'Not knowing superpriority for HOA liens in some states'],
            ),
            KnowledgeNode(
                concept_id="real_property_present_covenants",
                name="Present vs Future Covenants",
                subject="real_property",
                difficulty=4,
                rule_statement="Present covenants (seisin, right to convey, against encumbrances) breached at closing; future covenants (quiet enjoyment, warranty, further assurances) breached when evicted",
                elements=['Covenant of seisin', 'Right to convey', 'Against encumbrances', 'Quiet enjoyment', 'Warranty', 'Further assurances'],
                policy_rationales=['Title protection'],
                common_traps=['Missing when breach occurs for each type', 'Forgetting statute of limitations runs from breach', 'Confusing which covenants run with land'],
            ),
            KnowledgeNode(
                concept_id="real_property_options_rights_first_refusal",
                name="Options & Rights of First Refusal",
                subject="real_property",
                difficulty=4,
                rule_statement="Option gives holder right to purchase at set price; right of first refusal gives right to match third party offer; both subject to RAP in some jurisdictions",
                elements=['Option to purchase', 'Right of first refusal', 'RAP application', 'Commercial vs residential'],
                policy_rationales=['Flexibility in transactions'],
                common_traps=['Missing that commercial options often exempt from RAP', 'Forgetting ROFR must match all terms', 'Confusing option (set price) with ROFR (match offer)'],
            ),
            KnowledgeNode(
                concept_id="real_property_eminent_domain",
                name="Eminent Domain & Just Compensation",
                subject="real_property",
                difficulty=4,
                rule_statement="Government may take private property for public use with just compensation; public use broadly interpreted; compensation is fair market value",
                elements=['Public use requirement', 'Just compensation', 'Fair market value', 'Inverse condemnation'],
                policy_rationales=['Public benefit', 'Fairness to property owners'],
                common_traps=['Thinking public use requires actual public access (Kelo broadened)', 'Missing that consequential damages generally not compensable', 'Forgetting regulatory taking if goes too far (Penn Central)'],
            ),
        ]
        for node in concepts:
            self.nodes[node.concept_id] = node
